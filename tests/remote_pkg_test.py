...
class RemotePackageTest(TestCase):
    ...
    def test_open(self):
        ...
        pkg.save()
        for rem_pkg in pkg:
            self.assertFalse(os.path.exists(rem_pkg.path.full_path), 'Remote packages are not archived correctly')
        ...
        for rem_pkg in pkg2:
            ...
            self.assertTrue(os.path.exists(rem_pkg.path.full_path), 'Remote packages are not extracted correctly')
